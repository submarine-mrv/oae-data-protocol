...
- Use `/precommit-sweeper` before committing to audit changes
- When asked to commit: stage specific files, write clear messages, push only if asked

### Plan Mode
- Use plan mode for schema changes, multi-file refactors, and architectural decisions
- Skip plan mode for small fixes and single-file changes
...
**When to use beads:**
- Check `bd ready` at session start if the user doesn't specify a task
...
- Prefix suggestions with "[Claude suggestion]" in issue titles

**When NOT to use beads:**
- Trivial single-session tasks — use TodoWrite instead
